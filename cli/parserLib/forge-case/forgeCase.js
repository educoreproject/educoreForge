...
// forgeCase.js — the CASE forge bundle. Parses the IMS Global / 1EdTech CASE (Competencies and
// Academic Standards Exchange) OpenAPI 3.0 YAML schema and emits the UNIVERSAL FORGE PROPERTY
// CONTRACT (DESIGN §A/§B/§C/§D/§E/§F, DECISIONS §6-§12, §23-R3/R4) — the IDENTICAL shape forge-lif
// / forge-sif / forge-ceds emit, so CASE lands in golden interoperably.
//
// HARVESTED+ADAPTED from trackA forge-case (parser logic reused; node-shaping rewritten):
//   * trackA emitted private Case* nodes (CaseRoot/CaseClass/CaseProperty/CaseOptionSet/
//     CaseOptionValue) with HAS_CLASS/HAS_PROPERTY/_parentEdge plumbing and a hand-rolled
//     `|`-joined searchText.
//   * THIS module emits the canonical Dme* roles with DmeStandardRoot on top, builds searchText
//     via the ONE shared 1C builder (R4: empty -> ValidationError), captures CASE's harvested
//     `cedsGlobalIds` into a crossRefs JSON property AND normalizes them to canonical CEDS `cedsId`
//     (R3) with origin recorded in `cedsOriginalAnchorPropertyName` — this is the CASE→CEDS bridge
//     fuel the specified-bridge consumes at acceptance. Edges use the canonical ownership names
//     (HAS_CLASS/HAS_PROPERTY/HAS_OPTION_SET/HAS_VALUE) + REFERENCES (carrying CASE's anyOf/oneOf/
//     allOf polymorphism tag), all stamped provenanceTier 'structural'.
//
// CASE identity is STRUCTURAL (DESIGN §D case 3): stableUriPropertyName = 'casePath', a clean
// dotted locator the forge mints (root / class / class.property / class.property.optionSet /
// class.property.option.value). The stableId IS that casePath value.
//
// CASE-distinctive shape (faithful native scalars kept queryable): DType-stripped display names
// (rawSchema in `schemaName`), extension-point flag (`isExtensionPoint` — CF*ExtensionDType empty
// hooks), 1EdTech privacy annotations (`confidentiality` / `privacy`), and polymorphic $ref edges.
//
...
const STANDARD_KEY = 'case';
const STANDARD_SOURCE = 'CASE'; // === the registry standardName, EXACT (no toLower anywhere)
const STANDARD_DISPLAY = 'Competencies and Academic Standards Exchange';
const STABLE_URI_PROPERTY_NAME = 'casePath';
const EMBED_BATCH_SIZE = 128; // voyage batch ceiling headroom; bounds per-call payload.

// The mappingInstruction fields are DECLARED on the DmeStandardRoot (DECISIONS §12). CASE bridges TO
// the CEDS hub (impliedTargets ['CEDS']); its native CEDS anchor origin is the CEDS element id
// harvested from a property's description prose into cedsGlobalIds, normalized to canonical cedsId.
const caseMappingInstruction = {
...
const moduleFunction =
	...
	({ embedder } = {}) => {
		...
		// -----
		// casePath — mint the clean dotted structural stableId from ordered raw segments.
		//   Throws on a miss (R3-style: a node MUST have a clean stableId, never silent junk).
		const casePathFor = (segments) => {
			const result = normalize.buildCasePath(segments);
			if (result.error) {
				throw new Error(`forge-case casePath miss: ${result.error}`);
			}
			return result.casePath;
		};

		// -----
		// _id from natural keys (DESIGN §B "deterministic from natural keys"):
		//   <standardKey>:<casePath-minus-prefix>. The root uses a fixed natural key.
		const idFor = (casePath) => `${STANDARD_KEY}:${casePath.replace(/^case:/, '')}`;
		const ROOT_CASE_PATH = `${normalize.CASE_PATH_PREFIX}:root`;
		const ROOT_ID = `${STANDARD_KEY}:root`;

		// stableId minters (mirror the parser's structural shapes so Ref/ownership endpoints resolve).
		const classPath = (displayName) => casePathFor([displayName]);
		const propPath = (className, propertyName) => casePathFor([className, propertyName]);
		const optionSetPath = (className, propertyName) =>
			casePathFor([className, propertyName, 'optionSet']);
		const optionValuePath = (className, propertyName, value) =>
			casePathFor([className, propertyName, 'option', value]);

		// -----
		// crossRefsForCase — turn a property's harvested CEDS element ids into the DESIGN §E crossRefs
		...
		const crossRefsForCase = ({ cedsGlobalIds }) =>
			(cedsGlobalIds || []).map((rawId) => {
				const normalized = normalize.normalizeCedsId({ rawValue: rawId, kind: 'element' });
				if (normalized.error) {
					throw new Error(
						`forge-case R3 normalization miss: ${normalized.error} (cedsGlobalId='${rawId}')`,
					);
				}
				return {
					system: 'ceds',
		...
		//   Each node carries the universal contract MINUS embedding (added later). A casePath miss,
		...
		const buildContractGraph = ({ entities, metadata }) => {
			...
			//   stableId IS the casePath (CASE stableUriPropertyName). crossRefs / cedsId carried when
			...
			const makeNode = ({
				...
				if (!normalize.isCleanStableId(stableId)) {
					throw new Error(
						`forge-case: ${role} '${name}' has no clean stableId (casePath) — got '${stableId}'`,
					);
				}
				...
				const nodeProperties = {
					...
					role,
					casePath: stableId, // the stable identifier, under stableUriPropertyName
					searchText,
				};

				// crossRefs + canonical cedsId (R3) — only when the element harvested CEDS anchors.
				const crossRefs = crossRefsForCase({ cedsGlobalIds });
			...
			nodes.push({
				labels: [NODE_LABELS.FORGED_NODE, 'CaseRoot', DME_ROLES.STANDARD_ROOT],
				stableId: ROOT_CASE_PATH,
				role: DME_ROLES.STANDARD_ROOT,
				properties: {
					_id: ROOT_ID,
					_source: STANDARD_SOURCE,
					...
					mappingInstruction: JSON.stringify(caseMappingInstruction),
				},
			});

			// ---- DmeClass nodes (CASE classes — real + stub) ----
			classes.forEach((cls) => {
				const stableId = classPath(cls.displayName);
				makeNode({
					role: DME_ROLES.CLASS,
					perStandardLabel: 'CaseClass',
					stableId,
					...
					extraProps: {
						...
						isStub: !!cls.isStub,
						isExtensionPoint: !!cls.isExtensionPoint,
						...
						privacy: cls.privacy || '',
						confidentialityNormal: cls.confidentialityNormal || '',
					},
					structural: { parentId: ROOT_ID, depth: 1, path: cls.displayName },
				});
				// HAS_CLASS: root -> class (canonical ownership)
				addEdge(EDGE_TYPES.HAS_CLASS, ROOT_CASE_PATH, stableId);
			});
			...
			properties.forEach((prop) => {
				...
				makeNode({
					...
					extraProps: {
						...
						isExtensionPoint: !!prop.isExtensionPoint,
						confidentiality: prop.confidentiality || '',
			...
			// ---- REFERENCES edges (property -> target class; CASE polymorphism preserved) ----
			...
			optionSets.forEach((os) => {
				...
				makeNode({
					...
					perStandardLabel: 'CaseOptionSet',
					stableId,
					name: setName,
			...
			optionValues.forEach((ov) => {
				...
				makeNode({
					...
					perStandardLabel: 'CaseOptionValue',
					stableId,
					name: ov.value,
					description: '',
					searchTextElement: {
		...
		// =====================================================================
		// embedNodes — batched embedding pass over nodes (1C embedTexts). IDENTICAL to forge-lif.
		...
		const embedNodes = ({ nodes, nodeSubsetLimit }, callback) => {
			...
			const nextBatch = () => {
				...
				embedder.embedTexts({ texts }, (err, result) => {
					if (err) {
						callback(`forge-case embedNodes batch ${bi} failed: ${err}`);
						return;
					}
					embedCallCount++;
					...
					if (xLog && xLog.status) {
						xLog.status(
							`[forge-case] embedded batch ${bi}/${batches.length} (${batch.length} nodes)`,
						);
					}
					nextBatch();
				});
		...
		const forge = ({ sourcePath, owner, embedNodeLimit, skipEmbedding } = {}, callback) => {
			...
			taskList.push((args, next) => {
				parseCase({ sourcePath, xLog }, (err, parsed) => {
					if (err) {
						next(`forge-case parse: ${err}`);
						return;
					}
					next('', { ...args, parsed });
				});
			...
			taskList.push((args, next) => {
				...
				if (buildError) {
					next(`forge-case buildContractGraph: ${buildError}`);
					return;
				}
				xLog.status(
					`[forge-case] contract graph: ${graph.nodes.length} nodes, ${graph.edges.length} edges`,
				);
				next('', { ...args, graph });
			});
