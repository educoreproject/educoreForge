...
// normalize.js — CTDL-QData anchor + stableId normalization (R3, DESIGN §D/§E, DECISIONS §23-R3).
//
// CTDL-QData (Credential Transparency Description Language — Achievement Standards Network, Credential
...
// `evalCat:progression`). CTDL-QData's identity is therefore URI-bearing (DESIGN §D — the CEDS/URI
...
//
// CROSS-STANDARD NOTE: CTDL-QData carries ZERO CEDS anchors (verified against the source blob). Its only
...
// -----
// buildStableId — CTDL-QData stableId IS the term's native `@id` CURIE. Whatever the input, trim it; an
...
const buildStableId = ({ id } = {}) => {
	...
	if (cleaned === '') {
		return { error: `buildStableId: empty CTDL-QData @id` };
	}
	return { stableId: cleaned };
};

// isCleanStableId — the R3 predicate the test asserts against. A clean CTDL-QData stableId is a non-empty,
