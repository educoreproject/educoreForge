...
// parser.js — Open Badges 3.0 (1EdTech) OpenAPI 3.0 JSON schema reader.
//
// HARVESTED+ADAPTED from trackA forge-openbadges/lib/parser.js: the OpenAPI JSON extraction logic is
// proven and kept in substance — the components.schemas walk, the nine "plumbing" schema filter
// (OpenApi* / Imsx_* status envelope / ServiceDescriptionDocument / GetOpenBadgeCredentialsResponse
// / Proof), the $ref / anyOf / oneOf / allOf polymorphism collection (collectRefsAndEnums), enum ->
// optionSet/optionValue, the 1EdTech persistent-id annotations (x-class-pid on schemas, x-srcprop-pid
// on properties), the CEDS-element-URL harvest (extractCedsIds over description), and the stub-class
// synthesis for unresolved $ref targets. ADAPTED AWAY: trackA's node-shaping (it built private
// OpenBadges* nodes — OpenBadgesRoot/OpenBadgesClass/OpenBadgesProperty/OpenBadgesOptionSet/
// OpenBadgesOptionValue — with HAS_CLASS/HAS_PROPERTY/_parentEdge plumbing and a hand-rolled
// `|`-joined searchText, and used the graphForge parser contract (sourcePath, options, callback) ->
// {nodes, metadata}). THIS parser instead returns ONLY raw intermediate collections + the resolution
// maps + metadata; shaping into the universal property contract (the six Dme* roles, canonical
// ownership edges, searchText via the ONE shared builder, crossRefs, canonical cedsId,
// stableId/openbadgesPath) happens in forgeOpenbadges.js. The parser NEVER touches Neo4j,
// embeddings, or the contract.
//
// OpenBadges schema names have NO DType suffix (unlike CASE) — the display name IS the schema name.
// Source is JSON (not YAML), so no js-yaml dependency is required (the LIF pattern).
//
// Async style: a single callback (err, { entities, maps, metadata }); fs.readFileSync/JSON.parse is
// the only boundary and resolves immediately. No async/await, no try/catch-for-control-flow except
// the localized JSON parse boundary. camelCase only.
...
// Nine plumbing schemas (REST envelope / runtime cryptographic machinery) carry no data-model meaning.
...
//   would be OpenBadges' native cross-reference to CEDS (THIS source carries 0; path kept + tested).
...
//   { refs: [{ schemaName, polymorphism }], enums: [array-of-values] }. The VCDM 1.1-vs-2.0 oneOf
//   branch on credential @context resolves through this path.
...
const parseOpenbadges = ({ sourcePath, xLog }, callback) => {
	const log = (xLog && xLog.status) || (() => {});

	if (!fs.existsSync(sourcePath)) {
		callback(`OpenBadges OpenAPI JSON source not found: ${sourcePath}`);
		return;
	}
	...
	if (parseError) {
		callback(`Failed to read/parse OpenBadges JSON: ${parseError}`);
		return;
	}

	log(
		`[forge-openbadges/parser] parsing OpenAPI JSON (${(fs.statSync(jsonPath).size / 1024).toFixed(0)} KB)...`,
	);
	...
	log(
		`[forge-openbadges/parser] kept schemas: ${keptNames.length} (filtered plumbing: ${filteredCount})`,
	);
	...
	//   stub. OpenBadges schema names have no DType suffix, so displayName === schemaName. The forge
	//   mints the same openbadgesPath for both, so a REFERENCES edge always resolves to a real node.
	...
	// ------------------------------------------------------------
	// Raw intermediate collections (NO node shaping here — that is forgeOpenbadges.js's job)
	// ------------------------------------------------------------
	const classes = []; // real OpenBadges classes (one per kept schema)
	...
	keptNames.forEach((schemaName) => {
		...
		const displayName = schemaName; // no DType suffix in OpenBadges
		...
		Object.keys(classProps).forEach((propName) => {
			...
			// forge can mint the target class's openbadgesPath; stub targets become synthesized stubs.
			refs.forEach((oneRef) => {
			...
			properties.push({
				...
				cedsGlobalIds, // RAW native CEDS element ids harvested from prose (0 in this source)
	...
	[...stubNames].forEach((stubName) => {
		...
		const displayName = stubName;
		const reason = PLUMBING_SCHEMAS.has(stubName)
			? 'filtered as REST-envelope/cryptographic plumbing'
	...
	log(
		`[forge-openbadges/parser] classes=${stats.classes}(+${stats.stubClasses} stub), ` +
