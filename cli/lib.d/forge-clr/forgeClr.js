...
// forgeClr.js — the CLR forge bundle. Parses the IMS Global / 1EdTech CLR (Comprehensive Learner
// Record) v2.0 OpenAPI 3.0 JSON schema and emits the UNIVERSAL FORGE PROPERTY CONTRACT (DESIGN
// §A/§B/§C/§D/§E/§F, DECISIONS §6-§12, §23-R3/R4) — the IDENTICAL shape forge-case / forge-lif /
// forge-ceds emit, so CLR lands in golden interoperably.
//
// HARVESTED+ADAPTED from trackA forge-clr (parser logic reused; node-shaping rewritten):
//   * trackA emitted private Clr* nodes (ClrRoot/ClrClass/ClrProperty/ClrOptionSet/ClrOptionValue)
//     with HAS_CLASS/HAS_PROPERTY/_parentEdge plumbing and a hand-rolled `|`-joined searchText.
//   * THIS module emits the canonical Dme* roles with DmeStandardRoot on top, builds searchText via
//     the ONE shared 1C builder (R4: empty -> ValidationError), captures CLR's harvested
//     `cedsGlobalIds` into a crossRefs JSON property AND normalizes them to canonical CEDS `cedsId`
//     (R3). Edges use the canonical ownership names (HAS_CLASS/HAS_PROPERTY/HAS_OPTION_SET/HAS_VALUE)
//     + REFERENCES (carrying CLR's anyOf/oneOf/allOf polymorphism tag), all stamped provenanceTier
//     'structural'.
//
// CLR identity is STRUCTURAL (DESIGN §D case 3): stableUriPropertyName = 'clrPath', a clean dotted
// locator the forge mints (root / class / class.property / class.property.optionSet /
// class.property.option.value). The stableId IS that clrPath value.
//
// CLR-distinctive shape (faithful native scalars kept queryable): 1EdTech persistent ids
// (`persistentId` from x-class-pid on classes, x-srcprop-pid on properties), and polymorphic $ref
// edges. CLR is a W3C-VC credential schema and may reference external vocab (OpenBadges etc.); this
// is a STANDARD-PURE forge — it emits NO cross-standard edges. Any external target that is not a
// CLR schema becomes a synthesized stub class so the REFERENCES edge resolves internally.
//
// PURITY: buildContractGraph is a PURE, synchronous, deterministic function of the parsed source.
// forge() runs parse -> buildContractGraph -> embedNodes.
//
...
const STANDARD_KEY = 'clr';
const STANDARD_SOURCE = 'CLR'; // === the registry standardName, EXACT (no toLower anywhere)
const STANDARD_DISPLAY = 'Comprehensive Learner Record';
const STABLE_URI_PROPERTY_NAME = 'clrPath';
const EMBED_BATCH_SIZE = 128; // voyage batch ceiling headroom; bounds per-call payload.

// The mappingInstruction fields are DECLARED on the DmeStandardRoot (DECISIONS §12). CLR bridges TO
...
const moduleFunction =
	...
	({ embedder } = {}) => {
		...
		// -----
		// clrPath — mint the clean dotted structural stableId from ordered raw segments.
		//   Throws on a miss (R3-style: a node MUST have a clean stableId, never silent junk).
		const clrPathFor = (segments) => {
			const result = normalize.buildClrPath(segments);
			if (result.error) {
				throw new Error(`forge-clr clrPath miss: ${result.error}`);
			}
			return result.clrPath;
		};

		// -----
		// _id from natural keys (DESIGN §B "deterministic from natural keys"):
		//   <standardKey>:<clrPath-minus-prefix>. The root uses a fixed natural key.
		const idFor = (clrPath) => `${STANDARD_KEY}:${clrPath.replace(/^clr:/, '')}`;
		const ROOT_CLR_PATH = `${normalize.CLR_PATH_PREFIX}:root`;
		const ROOT_ID = `${STANDARD_KEY}:root`;

		// stableId minters (mirror the parser's structural shapes so Ref/ownership endpoints resolve).
		const classPath = (displayName) => clrPathFor([displayName]);
		const propPath = (className, propertyName) => clrPathFor([className, propertyName]);
		const optionSetPath = (className, propertyName) =>
			clrPathFor([className, propertyName, 'optionSet']);
		const optionValuePath = (className, propertyName, value) =>
			clrPathFor([className, propertyName, 'option', value]);

		// -----
		// crossRefsForClr — turn a property's harvested CEDS element ids into the DESIGN §E crossRefs
		...
		const crossRefsForClr = ({ cedsGlobalIds }) =>
			(cedsGlobalIds || []).map((rawId) => {
				const normalized = normalize.normalizeCedsId({ rawValue: rawId, kind: 'element' });
				if (normalized.error) {
					throw new Error(
						`forge-clr R3 normalization miss: ${normalized.error} (cedsGlobalId='${rawId}')`,
					);
				}
				return {
					system: 'ceds',
		...
		//   Each node carries the universal contract MINUS embedding (added later). A clrPath miss,
		...
		const buildContractGraph = ({ entities, metadata }) => {
			...
			//   stableId IS the clrPath (CLR stableUriPropertyName). crossRefs / cedsId carried when
			...
			const makeNode = ({
				...
				if (!normalize.isCleanStableId(stableId)) {
					throw new Error(
						`forge-clr: ${role} '${name}' has no clean stableId (clrPath) — got '${stableId}'`,
					);
				}
				...
				const nodeProperties = {
					...
					role,
					clrPath: stableId, // the stable identifier, under stableUriPropertyName
					searchText,
				};

				// crossRefs + canonical cedsId (R3) — only when the element harvested CEDS anchors.
				const crossRefs = crossRefsForClr({ cedsGlobalIds });
			...
			nodes.push({
				labels: [NODE_LABELS.FORGED_NODE, 'ClrRoot', DME_ROLES.STANDARD_ROOT],
				stableId: ROOT_CLR_PATH,
				role: DME_ROLES.STANDARD_ROOT,
				properties: {
					_id: ROOT_ID,
					_source: STANDARD_SOURCE,
					...
					mappingInstruction: JSON.stringify(clrMappingInstruction),
				},
			});

			// ---- DmeClass nodes (CLR classes — real + stub) ----
			classes.forEach((cls) => {
				const stableId = classPath(cls.displayName);
				makeNode({
					role: DME_ROLES.CLASS,
					perStandardLabel: 'ClrClass',
					stableId,
				...
				addEdge(EDGE_TYPES.HAS_CLASS, ROOT_CLR_PATH, stableId);
			});
			...
			// ---- REFERENCES edges (property -> target class; CLR polymorphism preserved) ----
			...
			optionValues.forEach((ov) => {
				...
				makeNode({
					...
					perStandardLabel: 'ClrOptionValue',
					stableId,
					name: ov.value,
					description: '',
					searchTextElement: {
		...
		// =====================================================================
		// embedNodes — batched embedding pass over nodes (1C embedTexts). IDENTICAL to forge-case.
		...
		const embedNodes = ({ nodes, nodeSubsetLimit }, callback) => {
			...
			const nextBatch = () => {
				...
				embedder.embedTexts({ texts }, (err, result) => {
					if (err) {
						callback(`forge-clr embedNodes batch ${bi} failed: ${err}`);
						return;
					}
					embedCallCount++;
					...
					if (xLog && xLog.status) {
						xLog.status(
							`[forge-clr] embedded batch ${bi}/${batches.length} (${batch.length} nodes)`,
						);
					}
					nextBatch();
				});
		...
		const forge = ({ sourcePath, owner, embedNodeLimit, skipEmbedding } = {}, callback) => {
			...
			taskList.push((args, next) => {
				parseClr({ sourcePath, xLog }, (err, parsed) => {
					if (err) {
						next(`forge-clr parse: ${err}`);
						return;
					}
					next('', { ...args, parsed });
				});
			...
			taskList.push((args, next) => {
				...
				if (buildError) {
					next(`forge-clr buildContractGraph: ${buildError}`);
					return;
				}
				xLog.status(
					`[forge-clr] contract graph: ${graph.nodes.length} nodes, ${graph.edges.length} edges`,
				);
				next('', { ...args, graph });
			});
