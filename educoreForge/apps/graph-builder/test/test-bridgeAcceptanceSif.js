...
const runComposeSection = () => {
	...
	harness.ok('BG-COMPOSE-SIF c the plugin, the recipes, the acceptance data and this suite are the WHOLE diff — tracked changes and untracked additions alike', (() => {
		...
		const PERMITTED_PATH_REGISTRY = [
			...
			{ pattern: /^recipes\/fourWithHubFourBridges\.recipe\.jsonc$/, why: 'RULING FJ-P5-1 (supervisor FROZEN_JOURNEY, 2026-08-29): the Phase 5 four-forge verification build declares the FOUR bridge plugins that can coexist in one manifest — sifCedsStandardPlugin, edfiCedsCrosswalkPlugin, edfiCedsDerivedPlugin, pescCedsDerivedPlugin. It is a THIRD recipe and therefore trips the row above BY DESIGN. Measured, not assumed: the only colliding pair among the five plugins is PESC two derived tiers, which share BOTH a standard pair AND a producerKind and so compose one relationshipSubject (vocabulary.js RELATIONSHIP_PRODUCER_SUFFIX is a three-row map: authored _exact, inferred _close, structural _struct). Ed-Fi two plugins do NOT collide — crosswalk is authored, derived is inferred. pescOptionSetCedsDerivedPlugin stays OUT of THIS recipe. CORRECTED 2026-08-29 (Phase 7, WILD_SHARD): this clause used to read "until Phase 7 puts the bridge name into relationshipSubject", which describes a design TQ REJECTED. Putting a bridge name into every relationship subject re-keys the ENTIRE lineage — every existing relationship block — and the constraint he set was verbatim "purely backward compatible - doesn\'t affect any other work". What Phase 7 actually shipped is the OPT-IN discriminator: a plugin DECLARES subjectDiscriminator and it is appended AFTER the producer suffix, so an undeclared subject is byte-identical and nothing re-keys. The five-bridge recipe named in its own row below is where that tier finally runs. This correction is recorded rather than quietly overwritten because a stale why is how a successor rebuilds the wrong design from a comment' },
			// ── PHASE 7 (WILD_SHARD, 2026-08-29) — FIVE ROWS, ONE PER FILE, NONE OF THEM A WIDENING ───────────
			// The row above says a third recipe must not arrive unremarked; a FOURTH is arriving and is remarked
			// here by name. The tempting single pattern /^recipes\/.*\.recipe\.jsonc$/ is REFUSED again, for the
			// reason FJ-P5-2 gave: it hands back exactly the blind spot individual naming exists to close.
			//
			// ALL FIVE WERE OBSERVED DOING THEIR JOB BEFORE THEY WERE BELIEVED: with these rows absent and the
			// five files present, this conjunct went RED (test-bridgeAcceptanceSif 95/95 → 93/95, conjuncts (a)
			// and (c)). (a) is the framework diff and is answered by the ruled re-anchor chain, not by a row.
			{ pattern: /^recipes\/fourWithHubFiveBridges\.recipe\.jsonc$/, why: 'the Phase 7 verification recipe — the FIRST recipe in the campaign able to declare all five bridge plugins, because pescOptionSetCedsDerivedPlugin now declares subjectDiscriminator and so composes a distinct relationship subject instead of colliding with its property-tier sibling. A FOURTH recipe, and therefore tripping the individually-named row above BY DESIGN' },
			{ pattern: /^apps\/graph-builder\/lib\/bridgeCollisionRule\.js$/, why: 'SPEC-phase7 §3.7 — the PRE-SPEND declaration-tuple rule, EXTRACTED to a pure module rather than left a closure inside build.js so its red twins exercise the rule itself and not a copy (the genesisGuard.js / recordDisposition.js / batchWindowVerdict.js precedent; build.js cannot be required from a suite without a forger, a replay manager and a docker-backed graph). It is NOT inside SEAM_PATH_LIST, which names apps/graph-builder/lib/build.js specifically rather than the directory, nor inside BG-COMPOSE-PESC (a)\'s six paths for the same reason — both measured with their own commands at Phase 7 entry' },
			{ pattern: /^apps\/graph-builder\/test\/test-bridgeCollisionRule\.js$/, why: 'the suite for the rule above — five conjuncts, five twins, every conjunct observed red, including the one nobody would think to write: that a describedBridgeList which is not an array is REFUSED BY NAME rather than returning the empty string, because a pre-spend check that cannot run must never read as "no collision" and wave the judge spend through' },
			{ pattern: /^lib\/vocabulary\/test\/test-subjectDiscriminator\.js$/, why: 'invariants I-A and I-F — the frozen five-tuple baseline composes byte-identically with no discriminator declared, and both readers handle discriminated, undiscriminated and MALFORMED-tail subjects. Its own permitted home: BG-NOSUB conjunct (ii) requires the lib/vocabulary/ diff to touch only vocabulary.js and lib/vocabulary/test/, which this file is inside, so it owes (ii) no re-anchor — measured with (ii)\'s own command, not predicted' },
			{ pattern: /^lib\/bridge-framework\/test\/test-bgSubjectDiscriminator\.js$/, why: 'invariant I-G and finding [C1] — all five shipped plugins register UNCHANGED with the contract row present and undeclared, a malformed discriminator is refused by name, and a row WITHOUT `optional` still refuses on absence. Its [C1] twin deletes the plain-optional branch and every shipped plugin is then refused with "missing required key \'subjectDiscriminator\'", which is precisely what a `required: false` row would have done: the walk has never read `required`' },
			...
			{ pattern: /^apps\/graph-builder\/test\/bridgeAcceptance\/batchCheckpoint\.js$/, why: 'RULINGS BS-8, BS-12, BS-13, B4R-5 and SABLE_RIVER stand-down-B4 disposition (b) — ALL named, per B4R-7/F1, because five rulings have touched this generator and a row naming one of them understates what a reviewer must hold it to. BS-12 made the instruments basis-conditional (judged-subset rows, the block-declared window, forensics over judged records only). BS-13 split orphan from abstention. B4R-5 puts the picks into the aggregate comparison line, purges derived vocabulary from standard-basis documents, and makes the SECTIONS 0-2 heading say what it covers. Disposition (b) amends the window row: "clean" for a SUPERSEDED document means the window IN FORCE WHEN THE BLOCK WAS FROZEN, so the rule moved to batchWindowVerdict.js and the runner released row is now reported as context governing FUTURE launches. BS-8: the batch document generator is GENERALISED rather than forked — its two hardcoded truth constants become declared data (referenceStoreFilePath / referenceBlockId / referenceRole) and the ROLE selects the vocabulary, so a SIF document says COMPARISON where a derived document says TRUTH. SIF has no answer key; a document that called a disagreement an error would be claiming more than its evidence supports' },
			...
			{ pattern: /^apps\/graph-builder\/test\/bridgeAcceptance\/batchWindowVerdict\.js$/, why: 'SABLE_RIVER stand-down-B4 disposition (b), ratified as code under B4R-3: which window a batch document may CLAIM. TWILIGHT_VALLEY raised the flaw in its own BS-12 three-source check rather than weakening it — demanding that a block window equal the runner CURRENT released row makes every SUPERSEDED document permanently NOT CLEAN, because nothing on disk records which window was in force when a block was frozen. batch-0 ran at 10 and was clean; BS-9 released 70; batch-0 went unclean retroactively by an edit to a file it has no relationship with. RULED: clean means the window IN FORCE AT FREEZE, the block header is the authority for a document, and the runner row governs FUTURE launches only. Extracted to a pure module for the BS-13 / recordDisposition.js reason — batchCheckpoint.js runs on require, so a suite requiring it would WRITE a document and a twin could only test a copy. NOT a loosening: caller-disagrees-with-block still FAILS, and that is the inequality where the row always did its real work' },
...
const runJudgedSubsetSection = () => {
	...
	// AMENDED under SABLE_RIVER's stand-down-B4 disposition (b), ratified as code under B4R-3. All THREE sources
	// are still read; what changed is which comparison is the CLAIM. BS-12's original demanded that the block's
	// window equal the runner's CURRENT released row, which made every superseded document permanently NOT CLEAN
	// — batch-0 ran at 10, BS-9 later released 70, and batch-0 went unclean by an edit to a file it has no
	// relationship with. The claim is now that the CALLER describes the window the BLOCK ACTUALLY RAN, with the
	// released row reported as context. This conjunct asserts the generator DELEGATES that decision to
	// batchWindowVerdict.js rather than comparing inline, so the twins in test-runnerContract.js (h) exercise the
	// very function the generator calls — batchCheckpoint.js runs on require and cannot be required from a suite.
	harness.ok('BG-JUDGED-SUBSET c the window is read from THREE independent sources — the block header (what ran), the runner row (what was released, now CONTEXT governing future launches) and --batchSize (what the caller believes) — the CLAIM being caller-agrees-with-block per disposition (b); the decision is DELEGATED to batchWindowVerdict.js, and the block header is PARSED, never inferred from the record count, which a short last page would make wrong exactly when it mattered', /batchWindowVerdictLib\.batchWindowVerdictFor\(\{ blockWindowLimit, callerWindowLimit: batchSize, releasedWindowLimit \}\)/.test(generatorText) && !/blockWindowLimit === releasedWindowLimit/.test(generatorText) && /PARTIAL_WINDOW_limit\(\\d\+\)_offset\(\\d\+\)/.test(generatorText));
