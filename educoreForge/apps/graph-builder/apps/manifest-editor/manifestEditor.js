...
function makeManifest({ name, description, recipeName, recipeRefId, standardsDatabase, members, storedRefId }) {
	...
	const add = ({ subjectRefId, kind, description: memberDescription, schemaBlock }, callback) => {
		...
		if (isBlank(subjectRefId)) {
			callback(
				`manifestEditor.add to ${selfName()}: a subjectRefId is REQUIRED — it is what the ` +
		...
		if (isBlank(memberDescription)) {
			callback(
				`manifestEditor.add '${subjectRefId}' to ${selfName()}: a description is REQUIRED ` +
		...
		if (typeof blockText !== 'string' || blockText.length === 0) {
			callback(
				`manifestEditor.add '${subjectRefId}' to ${selfName()}: the schemaBlock carries no ` +
		...
		const alreadyThere = members.filter((oneMember) => oneMember.subjectRefId === subjectRefId)[0];
		if (alreadyThere) {
			callback(
				`manifestEditor.add to ${selfName()}: subjectRefId '${subjectRefId}' is already ` +
		...
		standardsDatabase.saveBlock(
			...
			(saveErr, saveReport) => {
				if (saveErr) {
					callback(`manifestEditor.add '${subjectRefId}' to ${selfName()}: ${saveErr}`);
					return;
				}
				members.push({
					subjectRefId,
