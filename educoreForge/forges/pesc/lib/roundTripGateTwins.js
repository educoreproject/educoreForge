...
// roundTripGateTwins.js (forge-pesc) — the fault injections that prove each gate BITES (RT-10).
//
// DOCTRINE: a gate never observed failing is unproven. Every gate in
// gates/pescRoundTripGates.jsonc names a twin here; the test records a gate PASS-able only after
...
// probes in test/test-pescRoundTrip.js, which run the REAL canonicalizer, serializer, and diff
// against the fixture and (for the cheating detector) delete a REAL graph fact and watch the
// REAL diff name it LOST. Saying so here rather than letting the suite look stronger than it is.
//
...
const moduleFunction =
	...
	(unusedDeps = {}) => {
		const twinRegistry = {
			// G-1: the smallest fabrication there is — one statement PESC never made.
			...
			// G-7: the Phase-0 lesson made executable — two empty sets are identical, and a naive
			// comparison calls identical success. The floor must refuse it.
			...
			// G-12: one construct the instrument did not model, smuggled past the census.
