...
//   * DOCUMENTATION LITERALS are whitespace-collapsed (runs -> one space, trimmed) — the same
//     normalization the forge parser applies, so indentation can never register as loss.
...
const moduleFunction =
	...
	(unusedDeps = {}) => {
		...
		// -----
		// normalizeDocumentation — the whitespace ruling for prose literals: decode character
		// references, trim, collapse runs. Identical treatment on both sides.
		const normalizeDocumentation = (text) =>
			decodeXmlCharacterReferences(String(text == null ? '' : text))
				.replace(/\s+/g, ' ')
				.trim();
		...
		const canonicalizeXsdFileSet = ({ fileList } = {}, callback) => {
			...
			const nextFile = () => {
				if (fileIndex >= fileList.length) {
					...
					callback('', { statements: mergedStatements, stats: mergedStats });
					return;
				}
