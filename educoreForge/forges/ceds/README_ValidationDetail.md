...
the evidence behind each figure, the derived reading of the validator's source, the gate taxonomy, and
the commands to reproduce the run.

Nothing in this file is a summary. It is deliberately exhaustive and deliberately not the first thing
a reader opens.

**Superseded in one place by later evidence.** Section 3 below says the independent rdflib check did
not run. It has since been run, on 2026-08-09, against this run's own persisted artifacts, and it
agreed: 243,601 triples each side, strict 0/0, identical. The certificate carries that result. The
section is left as written because it correctly describes what the BUILD did.
