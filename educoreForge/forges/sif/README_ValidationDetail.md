# Validation detail — SIF

**This is the long argument behind `README_ValidationCertificate.md`.** Read the certificate first; it
carries the verdict, the counts, the known issues and the corpus record in about a page. Come here for
the all-pairs precedence mechanism, the derived reading of the canonicalizer's source, the full
modelling boundary, and the commands to reproduce the run.

Nothing in this file is a summary. It is deliberately exhaustive and deliberately not the first thing
a reader opens.

**One section has been overtaken by measurement.** Section 3 records that whether errata S-1
generalizes beyond `ValidMark` is *inferred, not verified*, and that the settling comparison had never
been run. It has since been run, on 2026-08-09. S-1 generalizes exceptionlessly: 6,586 container
elements have no export row, 1,887 of them repeatable, with zero exceptions in either direction. See
`system/management/zNotesPlansDocs/FINDING-sifExportOmitsContainerElements-080926.md` and the
certificate.
