...
const bridgeDeclaration = Object.freeze({
	...
	predicateSource: {
		...
		assertedBy: {
			...
			citation: "The SIF Implementation Specification asserts its CEDS alignment by ANNOTATION rather than by prose: it publishes NO sentence defining the CEDS ID column, in 4.3 or in any version since the column appeared in 4.0 (searched 2026-08-17: Introduction.html section 2 Document Conventions and DataModel.html sections 3.1.1-3.1.3, which define the Char and Type columns explicitly and contain zero occurrences of 'CEDS', across 4.0/4.1/4.2/4.3; References.html Appendix K cites CEDS nowhere; the published ImplementationSpecification.xlsx has no legend sheet and its shared-string table holds 'CEDS' exactly twice; the column header markup carries no link, title, footnote or legend anchor; A4L's own reader's guide at https://data.a4l.org/understanding-the-sif-specification/ walks the element table column by column and skips this one). In the forge's source snapshot the assertion is therefore the eight-column table header 'Name / Mandatory / Characteristics / Type / Description / XPath / CEDS ID / Format', repeated once per object table (159 lines carry 'CEDS ID'; the file holds 160 occurrences of 'CEDS' in total, and the 160th is the hedged element description named at the end of this citation, at ImplementationSpecification_031326.tsv line 9849 — so the header is the column's only DEFINITION-BEARING occurrence, not its only occurrence). WHAT THE STANDARD DOES ASSERT, machine-readably and unambiguously, is in its own GENERATED artifact: SIF_Message.xsd 4.3 (publishedXsdCrossCheck/01, generated from the canonical authoring spreadsheet per ruling R-SF-6) carries, per element, '<xs:appinfo><sifChar>O</sifChar><privacyRating /><cedsId>001490</cedsId><cedsURL>https://ceds.ed.gov/element/001490</cedsURL></xs:appinfo>' — 5,265 such <xs:appinfo> BLOCKS, of which 590 carry a POPULATED cedsId/cedsURL pair, each resolving one SIF element to exactly ONE CEDS element page, unqualified; the remaining 4,675 carry an empty, self-closing '<cedsId />' and assert nothing at all. The 590 populated pairs name 426 distinct CEDS elements. SIF_Message_Relaxed.xsd is a second generated copy carrying the same 590, so the tree-wide total is 1,180 across two files. CORRECTED UNDER RULING B4R-3 (measured 2026-08-17): an earlier form of this citation gave 5,265 as the PAIR count, which overstated the evidence base roughly ninefold — 5,265 is the block count, and 89 percent of those blocks carry no CEDS assertion. AND THE RATIO, stated because '5,265' read as though the annotation covered everything: 590 XSD-annotated elements against the 2,231 TSV-anchored SifField nodes this plugin actually asserts over is roughly 26 percent, so for the other ~74 percent the predicate rests on the eight-column table header alone — the header for which, as stated at the top of this citation, A4L publishes no definition. exactMatch is declared on those 590 UNHEDGED pairs (RULED SABLE_RIVER 2026-08-17, option (a); downgrading on the ABSENCE of hedging language was rejected as backwards — and RE-AFFIRMED on this corrected basis under RULING B4R-3, because the ruling turned on the absence of a relation word rather than on volume, so the number was wrong and the conclusion was not). PUBLISHED PROSE FOUND ALONGSIDE, added per that ruling and recorded in full because it does not agree with itself: A4L's fact sheet at https://files.a4l.org/home/FactSheets/Unity_and_CEDS_2022.pdf says 4.3 'has an element-to-element mapping to CEDS (v9 & v10)'; A4L's release announcements say elements are 'aligned to' and 'align with' CEDS; and ONE in-spec element description (EvaluationRecommendationType/Code, https://files.a4l.org/Implementation/NA/4.3/CommonTypes.html) reads 'This element corresponds to the global CEDS Id 000102' — the weaker verb the downgrade rule names, but written into a single element's free-text description rather than about the column, and on an element whose CEDS column cell is EMPTY, so it does not describe the annotation this plugin reads. THAT SENTENCE IS ALSO IN THE FORGE'S OWN SOURCE SNAPSHOT, at ImplementationSpecification_031326.tsv line 9849, and it is the 160th 'CEDS' referred to above: an earlier form of this citation called the 159 header hits 'the only occurrences of CEDS in the file' and then quoted this very exception from that same file. Corrected under RULING B4R-3, and named rather than quietly dropped, because it is the one sentence in the corpus that argues FOR closeMatch and a citation that loses it is a citation arguing only its own side. None of it is a column definition, and A4L publishes no wording that is.",
		},
	},
