...
// test-pesc260805EmbedTextOracle.js — PHASE P9 ORACLE EQUALITY for PESC 2026-08-05 (PLAN-forgeEmbedText-091426
// §8.3 R-ET-16/17, §8.4 R-ET-28/29; BRIEF-P9-sifPescDeclarations task 3). Forges the REAL PESC bundle over the
// committed 64-file XSD aggregate snapshot 01 with skipEmbedding true (no Docker, no Voyage, no network) and
// asserts:
...
//   HYGIENE  every text node carries exactly the labels [DmeEmbedText, ForgedNode, Pesc260805EmbedText],
...
//   PREFIX   every text stableId is the literal 'pesc260805:root/embedText/' + 64 hex (ONE slash); the root
//            is the declared literal 'pesc260805:root' (pescForgeDeclaration rootStableIdFrom 'declared'),
//            checked against the Pesc260805Root node the forge actually minted
//   DECLARED the shipped embedTextDeclaration is exactly TQ decision 1 (label, roles, list order,
//            effectiveDescription included)
// The oracle found NO multi-element declared value in PESC (COUNTS arrayExpanded {}), so there is no ARRAY
...
//   perturbedDeclarationList  a clone of the declaration drops 'effectiveDescription' from DmeProperty →
//                             ORACLE and DECLARED red
...
//
// Run: node forges/pesc260805/test/test-pesc260805EmbedTextOracle.js [-verbose]
...
const helpText = () => `
NAME
     ${moduleName} -- P9 oracle equality: PESC embed-text counts vs the P1 census, with red twins
...
const pescForgeDeclaration = require('../lib/pescForgeDeclaration');
const pescHooks = require('../lib/pescHooks')();
...
// THE ORACLE — evidence/P1-textListCensus-v2.log, section "=== pesc260805" (R-ET-17). absent = 5,694
// effectiveDescription; emptyAfterTrim = description 14,178 + documentation 14,178 + effectiveDescription
// 5,308; whitespaceDiffers = description 901 + documentation 901; arrayExpanded none.
const EMBED_TEXT_ORACLE = Object.freeze({
	embedTextNodeCount: 3547,
...
const ROLE_CENSUS_ORACLE = Object.freeze({
	DmeClass: Object.freeze({ nodes: 3272, withText: 3272, usesByPropertyName: Object.freeze({ description: 1611, documentation: 1611, name: 3272 }) }),
	DmeProperty: Object.freeze({ nodes: 17575, withText: 17575, usesByPropertyName: Object.freeze({ description: 5521, documentation: 5521, effectiveDescription: 12267, name: 17575 }) }),
	DmeOptionSet: Object.freeze({ nodes: 2422, withText: 2422, usesByPropertyName: Object.freeze({ description: 1959, documentation: 1959, name: 2422 }) }),
});
// the root is DECLARED (pescForgeDeclaration.rootStableId 'pesc260805:root'), so the text-id prefix is that
// literal + '/embedText/' — frozen here as a string, and the minted root is checked against it
const ROOT_STABLE_ID = 'pesc260805:root';
const ROOT_LABEL = 'Pesc260805Root';
...
const judgeDeclared = ({ forgeDeclaration }) => [
	{ label: 'DECLARED embedTextDeclaration equals TQ decision 1 (label, roles, list order, effectiveDescription included)', pass: JSON.stringify(forgeDeclaration.embedTextDeclaration) === JSON.stringify(RULED_EMBED_TEXT_DECLARATION), detail: JSON.stringify(forgeDeclaration.embedTextDeclaration) },
];
...
const perturbedDeclaration = Object.freeze({
	...pescForgeDeclaration,
	embedTextDeclaration: {
		embedTextLabel: pescForgeDeclaration.embedTextDeclaration.embedTextLabel,
		textPropertyListByRole: { ...pescForgeDeclaration.embedTextDeclaration.textPropertyListByRole, DmeProperty: ['name', 'description', 'documentation'] },
	},
});
...
const TWIN_LIST = Object.freeze([
	...
	{ twinName: 'doubleSlashJoin', forgeDeclaration: pescForgeDeclaration, mutationList: [{ modulePath: DERIVATION_FILE_PATH, find: DERIVATION_JOIN_FIND, replace: "const stableIdJoinText = rootStableId.endsWith('/') ? '/' : '//';" }], redConjunctNameList: ['PREFIX'], greenConjunctNameList: ['ORACLE', 'HYGIENE'] },
	{ twinName: 'carriedForbiddenProperty', forgeDeclaration: pescForgeDeclaration, mutationList: [{ modulePath: DERIVATION_FILE_PATH, find: DERIVATION_CARRIED_FIND, replace: `${DERIVATION_CARRIED_FIND} embeddingModelVersion: 'twin-carried',` }], redConjunctNameList: ['HYGIENE'], greenConjunctNameList: ['ORACLE', 'PREFIX'] },
]);
...
const forgeWith = ({ forgeDeclaration, mutationList }, callback) => {
	...
	const bundle = frameworkFactory({ embedder: null }).injectStandardHooks({ forgeDeclaration, hooks: pescHooks });
...
taskList.push((args, next) => {
	harness.section('REAL: the shipped PESC bundle declaration, forged with skipEmbedding true');
	forgeWith({ forgeDeclaration: pescForgeDeclaration, mutationList: [] }, (forgeError, forgeResult) => {
		harness.accepts('the real PESC forge succeeds', forgeError ? [forgeError] : []);
		if (forgeError) {
			next(`real forge refused: ${forgeError}`);
			return;
		}
		Object.keys(JUDGE_BY_CONJUNCT_NAME).forEach((conjunctName) =>
			JUDGE_BY_CONJUNCT_NAME[conjunctName]({ forgeResult, forgeDeclaration: pescForgeDeclaration }).forEach((oneCheck) => harness.ok(oneCheck.label, oneCheck.pass, oneCheck.detail)),
		);
		next('', args);
	});
});
