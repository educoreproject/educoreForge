# Validation detail — PESC260805

**This is the long argument behind `README_ValidationCertificate.md`.** Read the certificate first; it
carries the verdict, the counts and the known issues in about a page. Come here for the evidence: how
each figure was obtained, what was demonstrated rather than reasoned, the verbatim
`semanticValidationLimit`, the reproduce recipe, and the provenance of every correction.

Nothing in this file is a summary. It is deliberately exhaustive and deliberately not the first thing
a reader opens.

Sibling standards carry their own certificates at
`forges/{ceds,edfi,sif}/README_ValidationCertificate.md`.
