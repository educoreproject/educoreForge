...
// forgeCtdlqdata.js — the CTDL-QData forge bundle. Parses the Credential Engine CTDL Quantitative Data
// JSON-LD vocabulary (qdata.json, includemetaproperties=false stripped encoding) and emits the UNIVERSAL
// FORGE PROPERTY CONTRACT (the SAME contract forge-ctdl / forge-ceds / forge-edfi emit), so CTDL-QData
// lands in the validation graph interoperably. Clone-and-adapt of forge-ctdlasn (the enriched
// filter-and-reference template), itself a clone-and-adapt of forge-ctdl (the JSON-LD-family trailblazer).
//
// JSON-LD ROLE-MAPPING (CODE FACT — identical to forge-ctdl):
//   rdfs:Class        -> DmeClass          (11 native; the 7 ceterms:* overlap classes FILTERED OUT)
//   rdf:Property      -> DmeProperty       (82 native; the 24 ceterms:* overlap properties FILTERED OUT)
//   skos:ConceptScheme-> DmeOptionSet      (8)
//   skos:Concept      -> DmeOptionValue    (113)
//   schema root       -> DmeStandardRoot   (1)
//   (schema:* is NATIVE for QData: schema classes/properties ARE emitted as nodes above.)
//
// IDENTITY IS NATIVE-URI (DESIGN §D — the CEDS/URI model): every CTDL-QData term carries a globally-
// unique CURIE in `@id` (e.g. qdata:Observation, metricCat:Enrollment). The stableId IS that CURIE and
// stableUriPropertyName = 'uri'. No path minting is needed.
//
// CROSS-STANDARD REFERENCES — FILTER-AND-REFERENCE (WORKORDER Decision 1; generalized per FADED_FORGE
// ruling 2026-07-15; reciprocal of forge-ctdlasn's qdata handling per the LOCKED Phase 2 parameters).
// CTDL-QData's 31 `ceterms:*` overlap terms (7 classes incl ceterms:Credential + 24 properties) emit NO
// nodes, and QData also references CTDL-ASN terms (`ceasn:*`/`asn:*`). Any native QData term referencing a
// cross-standard term stashes a `{system, id, raw, locator}` crossRef on the referencing node (the parser
// sets `_crossRefs`; this forge merges it into the `crossRefs` JSON node property), keyed by target
// standard: ceterms -> system 'ctdl', ceasn/asn -> system 'ctdlasn'. NOTHING is silently dropped and NO
// cross-standard node or edge is emitted — STANDARD-PURE, mirroring exactly how forge-ctdl stamps CEDS
// anchors (`forgeCtdl.js:182-206, 283, 370-378`). The bridges to CTDL's `ceterms:*` nodes and to CTDL-ASN's
// terms are separate additive phases (WORKORDER Phase 3.5), never authored here.
//
// NO CEDS ANCHORS: CTDL-QData carries zero CEDS crosswalks, so this bundle authors NO CEDS bridge
...
// reached from forges/ctdlqdata/ as ../../lib. This is the ONE deliberate edit to an otherwise
// byte-faithful bundle (mirrors the forge-ctdl / forge-lif port).
...
const STANDARD_KEY = 'ctdlqdata';
const STANDARD_SOURCE = 'CTDLQData'; // === the registry standardName, EXACT (no toLower anywhere); distinct from 'CTDL' and 'CTDLASN'
const STANDARD_DISPLAY = 'Credential Transparency Description Language — Quantitative Data (CTDL-QData)';
...
const roleSpecByNativeLabel = {
	CtdlqdataClass: { role: DME_ROLES.CLASS, kind: 'class' },
	CtdlqdataProperty: { role: DME_ROLES.PROPERTY, kind: 'property' },
	CtdlqdataConceptScheme: { role: DME_ROLES.OPTION_SET, kind: 'optionSet' },
	CtdlqdataConcept: { role: DME_ROLES.OPTION_VALUE, kind: 'optionValue' },
};
...
// The mappingInstruction fields are DECLARED on the DmeStandardRoot (DECISIONS §12). CTDL-QData is an
...
const ctdlqdataMappingInstruction = {
	crosswalkPrefix: [],
...
const moduleFunction =
	...
	({ embedder } = {}) => {
		...
		const ROOT_STABLE_ID = 'ctdlqdata:root';
		...
		const stableIdFor = (rawId) => {
			...
			if (result.error) {
				throw new Error(`forge-ctdlqdata R3 stableId miss: ${result.error}`);
			}
			if (!normalize.isCleanStableId(result.stableId)) {
				throw new Error(
					`forge-ctdlqdata: produced an unclean stableId '${result.stableId}' from @id '${rawId}'`,
				);
			}
		...
		const buildContractGraph = ({ nodes: nativeNodes, metadata }) => {
			...
			const stats = {
				...
				crossRefBySystem: {}, // per-target-standard tally (ctdl + ctdlasn)
				...
				orphanAnchoredProperties: 0, // properties owned by 0 native class, anchored from root (QData: 5)
			...
			nativeNodes.forEach((nativeNode) => {
				if (nativeNode.label === 'CtdlqdataRoot') {
				...
				if (!spec) {
					throw new Error(`forge-ctdlqdata: unknown native CTDL-QData label '${nativeNode.label}'`);
				}
			...
			nodes.push({
				labels: [NODE_LABELS.FORGED_NODE, 'CtdlqdataRoot', DME_ROLES.STANDARD_ROOT],
				stableId: ROOT_STABLE_ID,
				role: DME_ROLES.STANDARD_ROOT,
				properties: {
					...
					// version-provenance stamp (spec §3.3, Phase A): always present post-stamping. CTDL-QData's
					...
					mappingInstruction: JSON.stringify(ctdlqdataMappingInstruction),
					// NOTE: no anchorForm and no cedsOriginalAnchorPropertyName — CTDL-QData carries no CEDS
			...
			nativeNodes.forEach((nativeNode) => {
				...
				// cross-standard crossRefs (Decision 1, generalized): merge the parser's ceterms + ceasn/asn
			...
			nativeNodes.forEach((nativeNode) => {
				...
				if (nativeNode._parentEdge) {
					...
					if (!canonical) {
						throw new Error(
							`forge-ctdlqdata: untranslated native parent edge type '${nativeNode._parentEdge.type}'`,
						);
					}
					addEdge(
						canonical,
				...
				(nativeNode.edges || []).forEach((nativeEdge) => {
					...
					if (!canonical) {
						throw new Error(
							`forge-ctdlqdata: untranslated native edge type '${nativeEdge.type}' (${nativeNode.id} -> ${nativeEdge.targetId})`,
						);
					}
			...
			nativeNodes.forEach((nativeNode) => {
				if (nativeNode.label !== 'CtdlqdataConceptScheme') {
					return;
				}
				const self = nativeIdToSelf[nativeNode.id];
				if (!self) {
			...
			// ---- ORPHAN-PROPERTY-ANCHOR pass (QData divergence from forge-ctdlasn, FADED_FORGE ruling
			// 2026-07-15): a native property owned by ZERO native class (no _parentEdge — its only domain(s)
			// were cross-standard, e.g. ceterms:*, or it declares no domain at all) would be unreachable via
			// graph traversal. Anchor each such orphan from the DmeStandardRoot via HAS_PROPERTY, exactly
			// parallel to the orphan-option-set pass. STRUCTURAL scaffolding (provenanceTier:'structural'),
			// NOT a fabricated domain claim: a real cross-standard domain, when present, is already preserved
			// as a crossRef; a genuinely domain-less property gets NO invented domain/crossRef — reachability
			// only. ASN had 0 such properties so this pass never fired there; QData has 5.
			nativeNodes.forEach((nativeNode) => {
				if (nativeNode.label !== 'CtdlqdataProperty') {
					return;
				}
				const self = nativeIdToSelf[nativeNode.id];
				if (!self) {
				...
				if (!nativeNode._parentEdge) {
					addEdge(EDGE_TYPES.HAS_PROPERTY, ROOT_STABLE_ID, self.stableId, 'root->orphanProperty');
					stats.orphanAnchoredProperties++;
				}
			});

			if (stats.danglingEdges.length > 0) {
				throw new Error(
					`forge-ctdlqdata: ${stats.danglingEdges.length} edge(s) had an unresolved endpoint (first: ${JSON.stringify(stats.danglingEdges[0])}) — never emit a partial edge`,
				);
			}
		...
		const perStandardLabelByRole = {
			[DME_ROLES.CLASS]: 'CtdlqdataClass',
			[DME_ROLES.PROPERTY]: 'CtdlqdataProperty',
			[DME_ROLES.OPTION_SET]: 'CtdlqdataConceptScheme',
			[DME_ROLES.OPTION_VALUE]: 'CtdlqdataConcept',
		};
		function perStandardLabelFor(role) {
			return perStandardLabelByRole[role] || 'CtdlqdataTerm';
		}
		...
		const embedNodes = ({ nodes, nodeSubsetLimit }, callback) => {
			...
			const nextBatch = () => {
				...
				embedder.embedTexts({ texts }, (err, result) => {
					if (err) {
						callback(`forge-ctdlqdata embedNodes batch ${bi} failed: ${err}`);
						return;
					}
					embedCallCount++;
					...
					if (xLog && xLog.status) {
						xLog.status(
							`[forge-ctdlqdata] embedded batch ${bi}/${batches.length} (${batch.length} nodes)`,
						);
					}
					nextBatch();
				});
		...
		const forge = ({ sourcePath, owner, embedNodeLimit, skipEmbedding } = {}, callback) => {
			...
			taskList.push((args, next) => {
				parseCtdlqdata({ sourcePath, xLog }, (err, parsed) => {
					if (err) {
						next(`forge-ctdlqdata parse: ${err}`);
						return;
					}
					// version-provenance stamp (BINDING spec §3.3, Phase A). CTDL-QData's source does not
			...
			taskList.push((args, next) => {
				...
				if (buildError) {
					next(`forge-ctdlqdata buildContractGraph: ${buildError}`);
					return;
				}
				xLog.status(
					`[forge-ctdlqdata] contract graph: ${graph.nodes.length} nodes, ${graph.edges.length} edges ` +
						...
						`${graph.stats.orphanAnchoredProperties} orphan-anchored properties, ` +
