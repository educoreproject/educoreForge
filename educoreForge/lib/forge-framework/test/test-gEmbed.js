...
frameworkMutationTwin({ registry: twinRegistry, gateId: GATE_ID, conjunctId: 'limitFilterThenSlice', twinName: 'sliceBeforeFilter', fileName: EMBED_FILE,
	find: "\t\t\tconst embeddableNodeList = nodes.filter((oneNode) => nonEmbeddableRoleList.indexOf(oneNode.role) === -1);\n\t\t\tconst targetNodeList =\n\t\t\t\tnodeSubsetLimit !== undefined && nodeSubsetLimit < embeddableNodeList.length\n\t\t\t\t\t? embeddableNodeList.slice(0, nodeSubsetLimit)\n\t\t\t\t\t: embeddableNodeList;",
	replace: "\t\t\tconst slicedFirst = nodeSubsetLimit !== undefined && nodeSubsetLimit < nodes.length ? nodes.slice(0, nodeSubsetLimit) : nodes;\n\t\t\tconst targetNodeList = slicedFirst.filter((oneNode) => nonEmbeddableRoleList.indexOf(oneNode.role) === -1);" });
frameworkMutationTwin({ registry: twinRegistry, gateId: GATE_ID, conjunctId: 'nonEmbeddableStaysInPlace', twinName: 'embedNonEmbeddableToo', fileName: EMBED_FILE, find: '\t\t\tconst embeddableNodeList = nodes.filter((oneNode) => nonEmbeddableRoleList.indexOf(oneNode.role) === -1);', replace: '\t\t\tconst embeddableNodeList = nodes.slice();' });
