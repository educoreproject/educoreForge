...
const threeConjunctList = [
	...
	// TABLE-DRIVEN, not "every edge has a provider" (RULING 2026-08-17). mappingProvider and subjectMatchField
	// are PRODUCER-CONDITIONAL at the write seam: required for an authored producer, FORBIDDEN for an inferred
	// one. This conjunct walks graphSeamRules' own disposition table rather than restating one half of it, so
	// it is correct for both producers and cannot drift from the seam it is checking. The INFERRED direction is
	// exercised in test-bgDerived.js, where a derived scenario exists.
	runConjunct({ conjunctId: 'd_matchBasisConsistentWithProviderTool', title: 'matchBasis is CONSISTENT with the producer-conditional edge properties and with mappingTool (forward table walk: an authored edge carries mappingProvider + subjectMatchField; tool present iff judged)', twinNameList: ['crosswalkWithoutProvider'], judge: succeeded((runReport, outcome) => { const dispositionRow = graphSeamRulesLib.EDGE_PROPERTY_DISPOSITION_BY_PRODUCER_KIND[runReport.producer]; const bad = edgesOf(outcome).filter((oneEdge) => graphSeamRulesLib.PRODUCER_CONDITIONAL_EDGE_PROPERTY_NAME_LIST.some((oneName) => (dispositionRow[oneName] === 'required') !== (oneEdge.properties[oneName] !== undefined && oneEdge.properties[oneName] !== null && oneEdge.properties[oneName] !== '')) || (oneEdge.properties.resolution === 'judged') !== (oneEdge.properties.mappingTool !== undefined)); return { pass: edgesOf(outcome).length > 0 && bad.length === 0, detail: `producer ${runReport.producer}; ${bad.length} bad of ${edgesOf(outcome).length}` }; }) }),
...
scenarioTwin({ registry: twinRegistry, gateId: 'BG-P6', conjunctId: 'a_judgedCarriesSourcePredicate', twinName: 'judgePredicateLands', leverKind: 'productionMutation', mutate: (scenario) => {
	...
	scenario.frameworkMutationList.push({ modulePath: path.join(scenarioLib.FRAMEWORK_DIR, JUDGE_FILE), find: '\treturn { sourceElementIdeaList: clientReturn.sourceElementIdeaList, candidateIdeaList: clientReturn.candidateIdeaList, sortedCandidateList: clientReturn.sortedCandidateList, chosenCardStableId: mapped.chosenCardStableId, choice: clientReturn.choice, category: clientReturn.category, rationale: clientReturn.rationale, confidence: band.confidence, discardedPredicateKeyCount };', replace: '\treturn { sourceElementIdeaList: clientReturn.sourceElementIdeaList, candidateIdeaList: clientReturn.candidateIdeaList, sortedCandidateList: clientReturn.sortedCandidateList, chosenCardStableId: mapped.chosenCardStableId, choice: clientReturn.choice, category: clientReturn.category, rationale: clientReturn.rationale, confidence: band.confidence, discardedPredicateKeyCount, predicateFromJudge: clientReturn.predicate };' });
	scenario.frameworkMutationList.push({ modulePath: path.join(scenarioLib.FRAMEWORK_DIR, FRAMEWORK_FILE), find: "\t\t\t\treturn { predicate: labelRow.disposition === 'tentative' ? labelRow.predicateIfPicked : labelRow.predicate, predicateAssertedBy: labelRow.predicateAssertedBy, sourceLabel: labelRow.sourceLabel };", replace: "\t\t\t\treturn { predicate: judged.predicateFromJudge !== undefined ? judged.predicateFromJudge : labelRow.disposition === 'tentative' ? labelRow.predicateIfPicked : labelRow.predicate, predicateAssertedBy: labelRow.predicateAssertedBy, sourceLabel: labelRow.sourceLabel };" });
} });
