...
// the bare `if` line also opens voteCandidatePool's own-texts check, so the find carries cardTextCosineFor's refusal line too
const MIXED_SUBJECT_REFUSAL_LINE = '\t\t\treturn { error: refuse.byName({ moduleName, what: `text record ${oneRecord.textStableId} describes ${oneRecord.sourceStableId}, but subjectTextRecordList[0]';
frameworkMutationTwin({ registry: twinRegistry, gateId: 'BG-ETS', conjunctId: 'o_cardTextRefusalsByName', twinName: 'mixedSubjectAccepted', fileName: RETRIEVAL_FILE, find: `\t\tif (oneRecord.sourceStableId !== subjectStableId) {\n${MIXED_SUBJECT_REFUSAL_LINE}`, replace: `\t\tif (false) {\n${MIXED_SUBJECT_REFUSAL_LINE}` });
