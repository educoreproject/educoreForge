...
const hygieneConjunctList = [
	...
	pureConjunct({ conjunctId: 'd_oneMachineGateSaysSoByName', title: "a one-machine / proxy gate says so BY NAME: test-bgP7.js labels its validator PROXY and names sssom-py's absence", twinNameList: ['proxyLabelDropped'], judge: (scenario) => { const text = scenario.p7TextOverride === undefined ? fs.readFileSync(path.join(__dirname, 'test-bgP7.js'), 'utf8') : scenario.p7TextOverride; return { pass: /PROXY \(framework header\/row\/curie_map validator; sssom-py absent on this machine/.test(text), detail: /PROXY/.test(text) ? 'labelled' : 'NOT labelled' }; } }),
];
...
scenarioTwin({ registry: twinRegistry, gateId: 'BG-HYGIENE', conjunctId: 'd_oneMachineGateSaysSoByName', twinName: 'proxyLabelDropped', leverKind: 'productionMutation', mutate: (scenario) => { scenario.p7TextOverride = fs.readFileSync(path.join(__dirname, 'test-bgP7.js'), 'utf8').replace(/PROXY \(framework header\/row\/curie_map validator; sssom-py absent on this machine[^)]*\)/g, 'validated'); } });
