...
// PORT PROVENANCE (hubReimplementation Phase 2, R-P2-2, 2026-08-03): RELOCATION of the
// incumbent npm/qtools-graph-forge-core/lib/vector-store/vector-store.js into the recreation
// tree, byte-faithful except the sqlite-instance require repointed at the recreation's own
// lib/sqlite-instance (same initDatabaseInstance contract). Brought across because the
// recreation's replay-engine sidecar seams (shapeNode emitEmbeddingRef / putDistinctNodeVectors
// / resolveNodeVectors) call exactly this interface, and Phase 2's vectorized hub block is the
// first block in the recreation whose inline-base64 form exceeds the V8 string ceiling.
	...
	(injectedDeps = {}) => {
		...
		const putVector = ({ modelVersion, inputText, vector }, callback) => {
			...
			taskList.push((args, next) => {
				...
				// ⟪P2-review S-4, port divergence⟫ INSERT OR IGNORE, so first-write-wins holds even
				// when two writers race past the SELECT above (two concurrent builds, or a build
				// racing the incumbent producer in the SAME shared store — the M-3 ruling makes
				// the store cross-producer BY DESIGN). A plain INSERT turned that race into a
				// primary-key constraint crash the comment above claimed could not happen; OR
				// IGNORE keeps the loser silent and the row identical (content-addressed key).
				const statement = `INSERT OR IGNORE INTO vectors
